...
class TestViews(TestCase):
    ...
    def setUp(self):
        ...
        # Setting up objects
        self.client = Client()
        self.user_1 = User.objects.create_user('karl', 'm@email.com',
                                               'chevyspass')
        self.customer_1 = Customer.objects.create(user=self.user_1,
                                                  name="Karl",
                                                  email="m@email.com")
        self.product_1 = Product.objects.create(name="Pen",
                                                price="5.00")
        self.order_1 = Order.objects.create(customer=self.customer_1,
                                            transaction_id=1)
        self.order_item_1 = OrderItem.objects.create(product=self.product_1,
                                                     order=self.order_1,
                                                     quantity=2)
    ...
    def test_ProductDetail_object_in_context(self):
        ...
        self.assertEqual(response_5.context[0]['object'],
                         Product.objects.get(id=self.product_1.id))
        # TODO debug assertIn
        # self.assertIn(Product.objects.get(id=self.product_1.id),
        # response_5.context[0].values())
        # self.assertIn(Product.objects.get(id=1),
        # response.context[0].values())
    ...
    def test_updateItem_action_add(self):
        ...
        response = self.client.post(self.update_item,
                                    json.dumps(request_1),
    ...
    def test_processOrder_order_completed(self):
        self.client.login(username='karl',
                          password='chevyspass')
        product_2 = Product.objects.create(name="Candy",
                                           price="10.0")
        order_item_2 = OrderItem.objects.create(product=product_2,
                                                order=self.order_1,
                                                quantity=1)
        user_form_data = {
            'form': {
        ...
        response = self.client.post(self.process_order,
                                    json.dumps(user_form_data),
                                    content_type='application/json')
